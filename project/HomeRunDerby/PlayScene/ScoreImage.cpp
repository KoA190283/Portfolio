...
ScoreImage::ScoreImage() : ScoreImage(nullptr)
{
}

ScoreImage::ScoreImage(PlayScene* pPlayScene)
{
	pPlayScene_ = pPlayScene;
}

//初期化
void ScoreImage::Initialize()
{
	//数字の画像データの読み込み
	...
	//文字の画像データの読み込み
	const std::string	FILE_PATH		= ExternalDataManager::GetFilePathPlaySceneImage(ExternalDataManager::PLAY_SCENE_IMAGE::SCORE);		//画像へのパス
	const int			IMAGE_HANDLE	= Image::Load(FILE_PATH);																			//画像のハンドル番号
	assert(IMAGE_HANDLE != Image::ERROR_CORD);

	//描画位置の設定
	transform_.position_	= DirectX::XMVectorSet(0.45f, -0.62f, 0, 0);	//描画位置
	transform_.scale_		= DirectX::XMVectorSet(0.5f, 0.5f, 0, 0);		//拡大サイズ
}

//更新
void ScoreImage::Update()
{
}

//描画
void ScoreImage::Draw()
{
